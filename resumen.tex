...
Sea $ v \in \float^{2}, v \neq 0 \ \frac{\partial f}{\partial v}(x_{0}, y_{0})$ \\
Observemos que, existe una recta que pasa por $(x_{0}, y_{0})$ y con vector director $v$ tal que $\delta(t) = tv + (x_{0}, y_{0})$ \\
Si $\exists \lim_{h \to 0} \left(\frac{f((x_{0}, y_{0}) + hv) - f(x_{0}, y_{0})}{h} \right) = \frac{\partial f}{\partial v}(x_{0}, y_{0})$ \\
(Preguntar esto el jueves)
...
\end{itemize}
Decimos que $f:D\subseteq \float^{2} \rightarrow \float$ es de clase $C^{1}(D)$ si $\exists f_{x}(x,y), f_{y}(x,y) \backslash (\forall (x, y) \in D)(f_{x}, f_{y}: D \subseteq \float^{2} \rightarrow \float)$ continua. \\
Decimos que f es de clase $C^{2}(D)$ si $f_{x x}$, $f_{xy}$, $f_{yx}$, $f_{yy}$ con $D \subseteq \float^{2} \rightarrow \float$ continua.
...
\begin{itemize}
    ...
    \item $\forall v \in \float^{2} \exists \frac{\partial f}{\partial v}(x,y) = <\triangledown f (x_{0}, y_{0}), v>$
\end{itemize} 
\textbf{Importante}: $<>$ es sinónimo del producto escalar.
