...
\begin{itemize}
    \item Máximo Local de f: Si $\exists r > 0 \ / \ si \ x \in (p_{0}-r, p_{0}+r) \implies f(x) \le f(p_{0})$
      \item Mínimo Local de f: Si $\exists r > 0 \ / \ si \ x \in (p_{0}-r, p_{0}+r) \implies f(x) \ge f(p_{0})$
...
Si $f:(a,b) \rightarrow \float$ tiene un extremo local en $p_{0}$ y $\exists f'(p_{0}) \\ \implies f'(p_{0}) = 0$ \\
\textbf{Lema}: Si $f:(a,b) \rightarrow \float \ / \ f'(p_{0}) > 0 \implies \exists r > 0$ tq 
\begin{itemize}
    \item Si $x \in (p_{0}-r, p_{0}) \implies f(x) \le f(p_{0})$
    \item Si $x \in (p_{0}, p_{0}+r) \implies f(x) \ge f(p_{0})$
\end{itemize}
\section*{Extremos Locales (en $\float^{n}$)}
Sea $f:D \subseteq \float^{n} \rightarrow \float$ decimos que $p_{0} \in D$
\begin{itemize}
    \item Máximo Local de f: Si $\exists r > 0 \ / \ si \ x \in \float_{r}(p_{0}) \implies f(x) \le f(p_{0})$
    \item Mínimo Local de: Si $\exists r > 0 \ / \ si \ x \in \float_{r}(p_{0}) \implies f(x) \ge f(p_{0})$
...
Si $f:D \subseteq \float^{n} \rightarrow \float$ tiene un extremo local en $p_{0}$ y $\exists \triangledown f(p_{0}) \implies \triangledown f(p_{0}) = (0, 0, ..., 0)$
\section*{Punto Crítico}
Llamamos Punto Crítico a un punto donde la \textbf{derivada no existe} o \textbf{la derivada es 0}. \\
Sea $f:D \subseteq \float^{n} \rightarrow \float$ y $p_{0} \in D$ es punto crítico de f si: 
\begin{itemize}
...
Finalmente, $(0,0)$ es un \textbf{Mínimo Local} porque no existe otra combinación tal que de exactamente igual a 0, y todas son mayores a esta. \\
2. $f(x,y) = x^{2} - y^{2}$ \\
