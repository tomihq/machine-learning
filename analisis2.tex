...
Si $f:[a,b] \ x [c, d] \rightarrow \float$ es Integrable, entonces se verifica que $\int \int_{R} f \ dA = \int^{b}_{a}(\int^{d}_{c}f(x,y) \ dy) \ dx =  \int^{d}_{c}(\int^{b}_{a}f(x,y) \ dx) \ dy $
