...
\begin{itemize}
    \item $\phi$: Es el ángulo que sube el vector.
    \item $\rho$: Magnitud.
...
\textbf{Teórica}: Ejercicio 1. Sea f(x,y) = $1-xy$ g(x,y) = $x^{2} + 2y^{2}$ \\
Se pide hallar los extremos absolutos de f restringidos a $g^{-1}(4) = \{(x,y) \in \float^{2}: x^{2}+2y^{2} = 4\}$ \\
Primero graficamos qué es la curva de nivel, en la práctica le llaman la región $A$. \\
La curva de nivel o región A es una elipse centrada en el $(0,0)$. 
\[\begin{minipage}[b]{0.6\textwidth}
    \includegraphics[width=\linewidth]{assets/mult_lagrange_1.png}
\end{minipage}\]
Como me piden extremos absolutos solamente me fijo el borde. \\
¿Es $g^{-1}(4)$ compacto? Sí. Recordemos que una región es compacta si es cerrada y acotada. \\
Es cerrada porque los puntos de la frontera son parte del conjunto $g^{-1}(4)$. Es acotada porque existe una bola donde podemos encerrar esta región. \\
Por lo tanto por el teorema de Weierstrass f tiene un mínimo y un máximo absoluto en la región $g^{-1}(4)$. \\
Ahora sí, usemos Lagrange: $\triangledown f(x,y) = (-y, -y)$, $\triangledown g(x,y) = (2x, 4y) \neq (0, 0)$ \\
\textbf{Importante}: Recordemos que el gradiente de g NO puede anularse, y se cumple solamente que no se anule si $(x,y) \neq (0,0)$. \\
...
En este caso, $\lambda \neq 0$ xq si fuese 0, (x,y) no vive en $x^{2} + 2y^{2} = 4$. Tambien vemos que $x \neq 0, y \neq 0$ xq si fuesen 0, no viven tampoco en $x^{2} + 2y^{2} = 4$ \\
Teniendo los datos de que $ x \neq 0, y \neq 0$ sé que podemos pasar dividiendo a estas. \\
Nos queda entonces $x^{2} = 2y^{2}$, reemplazando en 2) nos queda $x = \pm \sqrt{2}$, reemplazando ahora en $x^{2} = 2y^{2}$ nos queda $y \pm 1$ \\
Por lo tanto, los candidatos son: $\{(-\sqrt{2}, 1), (\sqrt{2}, 1), (-\sqrt{2}, 1), (\sqrt{2}, -1)\}$ \\
Evalúando todos los candidatos en f nos da que: 
\begin{itemize}
    \item El máximo de f restringido a $g^{-1}(4)$ es $(1+\sqrt{2})$ y se alcanza en $(-\sqrt{2}, 1)$ y $(\sqrt{2}, -1)$
    \item El mínimo de f restringido a $g^{-1}(4)$ es $(1-\sqrt{2}) $ y se alcanza en $(\sqrt{2}, 1)$ y $(-\sqrt{2}, -1)$
\end{itemize}
...
\[\begin{minipage}[b]{0.4\textwidth}
    \includegraphics[width=\linewidth]{assets/integral_acercarse.png}
\end{minipage}\]
...
\[\begin{minipage}[b]{0.5\textwidth}
    \includegraphics[width=\linewidth]{assets/integral_acercarse_2.png}
\end{minipage}\]
...
\[\begin{minipage}[b]{0.65\textwidth}
    \includegraphics[width=\linewidth]{assets/integral_acercarse_3.png}
\end{minipage}\]
...
\[\begin{minipage}[b]{0.65\textwidth}
    \includegraphics[width=\linewidth]{assets/comparacion_directa.png}
\end{minipage}\]
